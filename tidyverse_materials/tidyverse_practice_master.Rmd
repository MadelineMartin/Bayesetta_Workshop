...
readr::read_csv()
```{r}
dr_data_orig <- readr::read_csv("local_path/Bayesetta_Workshop/tidyverse_materials/bayesetta_tidyverse_dr_data.csv")
...
`percent_response` column with sigfigs to one decimal place.  

```{r}
dr_data <- dr_data %>%
  mutate(percent_response = signif(dr_data$response*100, 3))
dr_data
```
