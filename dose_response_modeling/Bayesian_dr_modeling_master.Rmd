...
prior2 <- dr_priors(
  ec50 = brms::prior(normal(-6.5, 3), nlpar = "ec50"),
  hill = brms::prior(normal(0.5, 1), ub = 0.01, nlpar = "hill"),
  bottom = brms::prior(normal(15,15), nlpar = "bottom"),
  top = 100
  )
prior2
```
...
```{r}
density_distributions(data_sample_priors2,
                      half_max_label = "ic50",
                      title_label = "Prior2 Density Distributions")
```
...
```{r}
compare_distributions(model1 = data_model, model2 = model2,
                      predictors_col_name = "drug",
                      half_max_label = "ic50")
```
